...
const Home = () => {
    ...
    const menuItems = routes.map(route => ({ key: route.key, label: <Link to={`/money/${route.slug}`} ><Button type='link'>{route.name}</Button></Link> }));

    // if(user) {
    //     menuItems.push({ key: 'sign-out', label: <Button type='link' onClick={signOutUser}>Sign Out</Button> });
    // } else {
    //     menuItems.push({ key: 'sign-in', label: <Link to="/money/auth"><Button type='link' >Sign In</Button></Link> });
    // }
